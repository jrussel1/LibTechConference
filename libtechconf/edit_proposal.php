<? 
	...
	if(isset($_POST['newPerson'])){
		echo '<script type="text/javascript">alert("' . $_POST['newPerson'] . '"); </script>';
	}
?>
...
<body>
	...
	<?
		...
		//Sequence for the creation of a new proposal
			...
			echo ("
			<form action='' method='post' id='New_Submission'>
				...
				<table>
					...
					<tr>
						<td><label id='lSubmission_Title'>Title:</label></td>						
						<td><input type='text' name='Submission_Title' /></td>
					</tr>
					<tr>
								<td><label id='lSubmission_Style'>Style:</label></td>
								<td><select name='Submission_Style' id='Submission_Style'>
								<option disabled='disabled' selected='selected'>Please select a type:</option>");
									$qry2="SELECT * FROM ".DB_DATABASE.".Session_Style;";
									$result2=mysql_query($qry2);
									if($result2) {
										while ( $row2 = mysql_fetch_array($result2) ) {
											echo("<option value='".$row2['Style']."' >".$row2['Style']."</option>");
										}
									}
								echo("</select></td>
							</tr>
							<tr>
								<td><label id='lDifficulty_Level'>Difficulty:</label></td>
								<td><select name='Difficulty_Level' id='Difficulty_Level'>
									<option disabled='disabled' selected='selected'>Please select a difficulty:</option>          
							...
							<tr>
								<td><label id='lMain_Presenter'>Main Presenter:</label></td>
								<td>
								<select name='Main_Presenter' id='Main_Presenter' class='chzn-limited-width'>
								...
								");
									...
									if($result3) {
										while ($row3 = mysql_fetch_array($result3)) {
											echo("<option value='".$row3['Person_First_Name']." ".$row3['Person_Last_Name']."'>".$row3['Person_First_Name']." ".$row3['Person_Last_Name']."</option>");
										}
									}
								echo("</select></td>
							</tr>
							<tr>
								...
								");
									...
									if($result4) {
										while ($row4 = mysql_fetch_array($result4)) {
											echo("<option value='".$row4['Person_First_Name']." ".$row4['Person_Last_Name']."'>".$row4['Person_First_Name']." ".$row4['Person_Last_Name']."</option>");
										}
									}
								echo("</select></td>
							</tr>
							<tr>
								<td></td><td><input type='button' id='hideshow' value='Add New Presenter'></td>
							</tr>
							<tr>
								<td colspan=2><div id='content' style='display:none;'>
								<table>
									<tr>
										<td>First Name:</td>
									...
									<tr>
										<td>Last Name:</td>
										<td><input type='text' name='lastName'></td></td>
									</tr>
									<tr>
